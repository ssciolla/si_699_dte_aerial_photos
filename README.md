...
Besides the dependencies passed on to it by extract_using_pypdf.py and georeference_links.py (see below), the script uses no additional third-party libraries. Local libraries referenced include the aforementioned scripts and additional function file, `misc_functions.py`, which contains helper functions utilized by multiple scripts. The sys, json, csv, and copy standard Python libaries are also used.
